...
const NOTE_TYPES = new Set(['normal', 'manzana', 'pareja']);
...
// Couples / Relationship features (V2_WEB)
const COUPLES_COLLECTION = process.env.FS_COUPLES_COLLECTION || 'Couples';
const COUPLE_MEMBERS_COLLECTION =
  process.env.FS_COUPLE_MEMBERS_COLLECTION || 'CoupleMembers';
const GOALS_COLLECTION = process.env.FS_GOALS_COLLECTION || 'CoupleGoals';
const CHECKINS_COLLECTION = process.env.FS_CHECKINS_COLLECTION || 'CoupleCheckins';
const ROUTINES_COLLECTION = process.env.FS_ROUTINES_COLLECTION || 'CoupleRoutines';
const ROUTINE_LOGS_COLLECTION =
  process.env.FS_ROUTINE_LOGS_COLLECTION || 'CoupleRoutineLogs';
const MESSAGES_COLLECTION = process.env.FS_MESSAGES_COLLECTION || 'CoupleMessages';
...
router.get('/couples', async (req, res) => {
  ...
  if (!database) return;
  try {
    const snapshot = await database.collection(COUPLES_COLLECTION).limit(50).get();
...
router.post('/couples', async (req, res) => {
  ...
  try {
    ...
    const docRef = await database.collection(COUPLES_COLLECTION).add(payload);
    res.json({ status: 'ok', data: { id: docRef.id, ...payload } });
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message });
  }
});
...
router.get('/couples/:id', async (req, res) => {
  ...
  if (!database) return;
  try {
    const doc = await database.collection(COUPLES_COLLECTION).doc(req.params.id).get();
    if (!doc.exists) {
      return res.status(404).json({ status: 'error', message: 'Pareja no encontrada' });
    }
...
router.patch('/couples/:id', async (req, res) => {
  const database = ensureDb(res);
  if (!database) return;
  try {
    const update = { ...req.body, updatedAt: nowIso() };
    await database.collection(COUPLES_COLLECTION).doc(req.params.id).set(update, { merge: true });
    const doc = await database.collection(COUPLES_COLLECTION).doc(req.params.id).get();
    res.json({ status: 'ok', data: { id: doc.id, ...doc.data() } });
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message });
...
router.post('/couples/:id/members', async (req, res) => {
  const database = ensureDb(res);
  if (!database) return;
  try {
    ...
    const docRef = await database.collection(COUPLE_MEMBERS_COLLECTION).add(payload);
    res.json({ status: 'ok', data: { id: docRef.id, ...payload } });
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message });
  }
...
router.post('/couples/:id/goals', async (req, res) => {
  const database = ensureDb(res);
  if (!database) return;
  try {
    ...
    const docRef = await database.collection(GOALS_COLLECTION).add(payload);
    res.json({ status: 'ok', data: { id: docRef.id, ...payload } });
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message });
  }
...
router.patch('/goals/:id', async (req, res) => {
  ...
  try {
    const update = { ...req.body, updatedAt: nowIso() };
    if (update.status && !GOAL_STATUSES.has(update.status)) {
      update.status = 'active';
    }
    await database.collection(GOALS_COLLECTION).doc(req.params.id).set(update, { merge: true });
    const doc = await database.collection(GOALS_COLLECTION).doc(req.params.id).get();
    res.json({ status: 'ok', data: { id: doc.id, ...doc.data() } });
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message });
  }
...
router.get('/couples/:id/checkins', async (req, res) => {
  const database = ensureDb(res);
  if (!database) return;
  try {
    let query = database
...
router.post('/couples/:id/checkins', async (req, res) => {
  const database = ensureDb(res);
  if (!database) return;
  try {
...
router.get('/couples/:id/routines', async (req, res) => {
  const database = ensureDb(res);
  if (!database) return;
  try {
    const snapshot = await database
...
router.post('/couples/:id/routines', async (req, res) => {
  const database = ensureDb(res);
  if (!database) return;
  try {
    ...
    const payload = {
      ...
      title: String(title).trim(),
      description: description || '',
      frequency: ROUTINE_FREQUENCIES.has(frequency) ? frequency : 'weekly',
      createdAt: nowIso(),
      updatedAt: nowIso(),
    };
    const docRef = await database.collection(ROUTINES_COLLECTION).add(payload);
    res.json({ status: 'ok', data: { id: docRef.id, ...payload } });
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message });
  }
...
router.get('/routines/:id/checkins', async (req, res) => {
  const database = ensureDb(res);
  if (!database) return;
  try {
    const snapshot = await database
      .collection(ROUTINE_LOGS_COLLECTION)
...
router.post('/routines/:id/checkins', async (req, res) => {
  const database = ensureDb(res);
  if (!database) return;
  try {
    const { date } = req.body || {};
    const dateIso = toISODate(date) || nowIso();
    const routineRef = database.collection(ROUTINES_COLLECTION).doc(req.params.id);
    const routineDoc = await routineRef.get();
    if (!routineDoc.exists) {
      return res.status(404).json({ status: 'error', message: 'Rutina no encontrada' });
    }
    const routineData = routineDoc.data() || {};
    const last = toDateOnly(routineData.lastCheckinAt);
    const current = toDateOnly(dateIso);
    let nextStreak = 1;
    if (last && current) {
      const delta = diffDays(last, current);
      if (delta === 0) {
        nextStreak = routineData.streak || 1;
      } else if (delta === 1) {
        nextStreak = (routineData.streak || 0) + 1;
      }
    }
    const logPayload = {
      routineId: req.params.id,
      coupleId: routineData.coupleId || null,
      date: dateIso,
      createdAt: nowIso(),
    };
    await database.collection(ROUTINE_LOGS_COLLECTION).add(logPayload);
    await routineRef.set(
      {
        lastCheckinAt: dateIso,
        streak: nextStreak,
        updatedAt: nowIso(),
      },
      { merge: true }
    ...
    res.json({
      status: 'ok',
      data: { ...logPayload, streak: nextStreak },
    });
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message });
  }
...
router.post('/couples/:id/messages', async (req, res) => {
  const database = ensureDb(res);
  if (!database) return;
  try {
    const { text, author } = req.body || {};
    if (!text || !String(text).trim()) {
      return res.status(400).json({ status: 'error', message: 'Texto requerido' });
    }
    const payload = {
      coupleId: req.params.id,
    ...
    const docRef = await database.collection(MESSAGES_COLLECTION).add(payload);
    await notifyEntityCreated({
      title: 'Nuevo mensaje',
      body: truncateText(payload.text, 80),
      data: { coupleId: req.params.id, action: 'message' },
    });
    res.json({ status: 'ok', data: { id: docRef.id, ...payload } });
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message });
